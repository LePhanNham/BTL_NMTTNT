...
class state:
    def __init__(self, board, state_parent, list_check_point):
        '''lưu trữ bảng hiện tại và trạng thái cha của trạng thái này'''
        self.board = board
        self.state_parent = state_parent
        self.cost = 1
        self.heuristic = 0
        self.check_points = deepcopy(list_check_point)
    ''' HÀM ĐỆ QUY ĐỂ QUAY LẠI TRẠNG THÁI ĐẦU TIÊN NẾU TRẠNG THÁI HIỆN TẠI LÀ MỤC TIÊU '''
    def get_line(self):
        '''sử dụng vòng lặp để tìm danh sách các bảng từ trạng thái bắt đầu đến trạng thái này'''
        if self.state_parent is None:
            return [self.board]
        return (self.state_parent).get_line() + [self.board]
    ''' tính toán hàm đánh giá heuristic được sử dụng cho thuật toán A* '''
    def compute_heuristic(self):
        ...
        if self.heuristic == 0:
            ...
            # self.heuristic = self.cost + sum(abs(list_boxes[i][0]- self.check_points[i][0]) + abs(list_boxes[i][1] - self.check_points[i][1]) for i in range(len(list_boxes)))
        return self.heuristic
    ''' nạp chồng toán tử cho phép lưu trữ các trạng thái trong hàng đợi ưu tiên'''
...
def determine_move_direction(pos1, pos2):
    if pos1[0] < pos2[0]:
        return "down"
    elif pos1[0] > pos2[0]:
        return "up"
